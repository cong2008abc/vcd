...
static inline float diff(const cv::Mat &img3f, int x1, int y1, int x2, int y2)
{
	const cv::Vec3f &p1 = img3f.at<cv::Vec3f>(y1, x1);
	const cv::Vec3f &p2 = img3f.at<cv::Vec3f>(y2, x2);
...
*/
int SegmentImage(const cv::Mat &_src3f, cv::Mat &pImgInd, double sigma, double c, int min_size)
{
	CV_Assert(_src3f.type() == CV_32FC3);
	int width(_src3f.cols), height(_src3f.rows);
	cv::Mat smImg3f;
	GaussianBlur(_src3f, smImg3f, cv::Size(), sigma, 0, cv::BORDER_REPLICATE);
