...
cv::Mat cut_percent(const cv::Mat &src, float percent) {
    ...
    int left = (src.cols - n_w) / 2;
    int top = (src.rows - n_h) / 2;
...
//        noise_img(src);
        ...
        k[3] = cut_percent(src, 0.9);
        //k[4] = cut_percent(src, 0.8);
        k[4] = smooth(src, 7);
        k[5] = noise_img(src, 0.1);

        char new_name[64];
        int num = get_num(file_name.c_str());
        for (int i = 0; i < 6; ++i) {
            sprintf(new_name, "/mnt/share/image_db4/%d_%d.jpg", num, i + 1);
            printf("%s\n", new_name);
            //show_mat(k[i]);
